...
class ImageScroller(SampleBase):
    ...
    def run(self):
        ...
        #Se configura el buffer para la pantalla y obtiene los pixeles de ancho y alto
        double_buffer = self.matrix.CreateFrameCanvas()
        img_width, img_height = self.image.size
        
        #Toma el valor del tiempo de los argumentos
        tiempo=self.parser.parse_args().time
        #La posición inicial de los pixeles de la imagen se encuentran vitualmente abajo de la pantalla
        ypos = -img_height
        #Mientras la variable sea menor que la altura de la imagen, ésta subirá pixel a pixel
        while ypos < 0: #Número de recorridos
            ypos += 1
