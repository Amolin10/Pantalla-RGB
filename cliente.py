...
############################ Datos #########################################
#Esta clase es una estructura para almacenar los parámetros para presentar una imagen en la pantalla informativa.
#   Los parámetros son: 
#       Número de imagen
#       Ruta de imagen
#       Efecto de entrada
...
class Bienvenida:    
    ...
    def cargar(self):
        ...
        rgbImagen = rgbImagen.resize((650, 600))
...
class Opciones:
    ...
    def abrir_archivo(self): 
        ...
        if os.path.exists(f'{ruta_configuracion}/configuracion.txt'):
            ...
            messagebox.showwarning("Pantalla RGB", "Error al cargar la configuración. \nSeleccione una ruta correta.")         
...
class Resumen:
    ...
    def establecer_ruta(self, ruta):
        ...
        print('ruta de cargar: ' + self.ruta_guardar)
    ...
    def enviar_configuracion(self):
        if self.ruta_guardar == '':
            ...
            for folder, subfolders, files in os.walk(f'{self.ruta_guardar}'):
                ...
                for file in files:
                    if file.endswith('.txt') or file.endswith('.ppm'):
                        ...
                        print(f'--------ruta {os.path.join(folder, file), os.path.relpath(os.path.join(folder,file), dir_zip)}')
            dir_config.close()
            ...
            try:
                ...
                messagebox.showinfo("Pantalla RGB", "Se ha enviado la configuracion")
            except:
                messagebox.showwarning("Pantalla RGB", "No se ha podido establecer conexión")
